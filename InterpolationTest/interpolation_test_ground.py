...
def test_interpolation(Traces, efield_interpolated):
    ...
    for i in range(ncross_check):
        ...
        if(Load):
            ...
            if(max(etot)>1e4):
                print(max(etot), max(cross_tot[i]))
            ...
            Etot_sim = max(cross_tot[i])
            Etot_scaled = max(etot)
           # print(Etot_sim, Etot_scaled)
            
            error_peak.append((Etot_sim - Etot_scaled)/Etot_scaled)
            rm_peak.append(Etot_scaled)
            zh_peak.append(Etot_sim)
    ...
    return error_peak, rm_peak, zh_peak
